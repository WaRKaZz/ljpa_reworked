...
2. **4D.2: One operator-reviewed PDF — profile rebuilt and guardrails verified.** Canonical `resources/profile.md` was rebuilt from the two local source CV PDFs and now declares structured general information, summary, experience, education, skills, certifications, languages, and projects. Deterministic validation requires core sections before any LLM call and requires generated resume coverage for every applicable declared resume section. The verified backup is retained; one final evaluator+generator PDF run for vacancy ID 1 remains, followed by Ivan's visual acceptance. Do not submit, email, message, collect LinkedIn, run JobSpy, or run the full pipeline.
