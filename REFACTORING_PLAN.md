...
2. **4D.2: One operator-reviewed PDF — detailed-contract implementation complete, awaiting visual acceptance.** Implemented structured in-memory evaluation inclusion plan (`required_profile_sections`, `prioritized_facts`, `missing_mandatory_facts`) in `BasicEvaluationCrewAI`, generator prompt requirement for 3 bullet points per entry, categorized skills, no invention, deterministic post-render PDF layout verification (`validate_pdf_page_layout`), and no-retry on validation failures. Restored baseline DB, executed exactly one evaluator and generator cycle for vacancy ID 1, and created replacement PDF `resources/resumes/resume_1_a3c7ec07.pdf`. Backup preserved and parent Stage 4D.2 remains incomplete awaiting Ivan's visual acceptance.
