...
2. **4D.2: One operator-reviewed PDF — in progress / replacement authorized.** Profile RAG/Knowledge has been replaced with direct full-file `candidate_profile` task inputs in both resume crews. Canonical `data/app.db` was restored from verified backup `data/app.db.stage-04d2-20260811T034454Z.bak`; the rejected PDF was removed. Before the one authorized replacement generator-only run for vacancy ID 1, the legacy real CrewAI test probe must be made hermetic. Keep the backup and leave 4D.2 incomplete until Ivan visually accepts the replacement PDF. Do not submit, email, message, collect LinkedIn, run JobSpy, or run the full pipeline.
