...
2. **4D.2: One operator-reviewed PDF — replacement rendered / awaiting Ivan's visual acceptance.** Replaced legacy real CrewAI test probe with a hermetic mock-based test. Executed exactly one generator-only run for vacancy ID 1 and `save_resume()`, producing valid PDF `resources/resumes/resume_1_b2908dbf.pdf` (34,612 bytes) and `Resume` DB record ID 1. Canonical `data/app.db` has 12 vacancies and 1 resume; `PRAGMA integrity_check` is ok. Backup `data/app.db.stage-04d2-20260811T034454Z.bak` remains verified and preserved. Stage 4D.2 remains in progress awaiting Ivan's visual review.
