...
### 4D: RenderCV output and one operator-reviewed PDF — **complete / verified by operator**
...
2. **4D.2: One operator-reviewed PDF — complete / accepted by operator.** Canonical `resources/profile.md` was rebuilt from the two local source CV PDFs and now declares structured general information, summary, experience, education, skills, certifications, languages, and projects. Deterministic validation requires core sections before any LLM call and requires generated resume coverage for every applicable declared resume section. A new RenderCV resume was saved for vacancy ID 1 as Resume ID 2 with a collision-safe relative PDF path and `rendered_at`; the persisted PDF has two pages with 3472 and 3055 extracted characters. Ivan accepted the rendered layout after removing connection icons and the `Last updated` footer. No application submission, email, Telegram, LinkedIn collection, JobSpy, or full pipeline was run.
...
4. **4D.4: RenderCV layout retry orchestration — complete / verified by operator.** Added production orchestration helper `crewai_generate_resume_with_retry()` that renders candidates to temporary PDFs and executes at most one corrective retry using factual layout feedback with numeric deficit/excess instructions. `save_resume()` now accepts pre-rendered temporary PDFs to eliminate redundant rendering passes while preserving atomic cleanup. Updated `main.py` and real Stage 4C smoke test to use the new production orchestration seam. Verified with hermetic unit test suite `tests/test_stage4d_rendercv_layout_retry.py` (4 passed), real smoke test `tests/test_stage4c_real_smoke.py` (1 passed), full pytest suite (120 passed), Ruff, compileall, and git diff checks.
5. **4D.5: Detailed layout retry prompt instructions — complete / verified by operator.** Enhanced `_format_numeric_layout_feedback()` in `src/ljpa_reworked/crew_workflow.py` to structure retry layout feedback with explicit observed page counts, range rules, numeric deltas targeting range midpoints (3387 / 1500), allowed fields in priority order (Summary, Skills, bullets), forbidden changes (no fake roles/dates/history), preserved facts/schema/section order, RenderCV entry movement rules (`allow_page_break_in_entries: false`), and raw JSON output reminders. Verified with focused unit test suite `tests/test_stage4d_rendercv_layout_retry.py` (9 passed), full pytest suite (159 passed), Ruff, compileall, and git diff checks.
6. **4D.6: Prior JSON resume snapshot in retry prompt — complete / verified by operator.** Extended `crewai_generate_resume()` with optional `prior_resume_json` parameter. When the first RenderCV PDF fails page budget in `crewai_generate_resume_with_retry()`, the first parsed `ResumeCrewAI` is serialized to raw JSON (`model_dump(mode="json")` / `json.dumps(..., ensure_ascii=False)`) and passed to the second gateway request alongside detailed numeric layout instructions. Prompt delimits `PREVIOUS RESUME JSON`, instructs preserving verified facts and schema while modifying only necessary text to meet layout corrections. Verified with hermetic unit test suite `tests/test_stage4d_rendercv_layout_retry.py` and `tests/test_stage4e_resume_gateway_adapter.py` (13 passed), full pytest suite (161 passed), Ruff, compileall, and git diff checks.
