...
async def fill_login_form(page: Page, email: str, password: str) -> bool:
    ...
    try:
        email_locator = page.locator(
            "input[type='email']:visible, #username:visible, input[name='session_key']:visible"
        )
        if await email_locator.count() > 0:
            ...
            password_locator = page.locator(
                "input[type='password']:visible, #password:visible, input[name='session_password']:visible"
            )
            await password_locator.first.fill(password)

            submit_btn = page.locator(
                "button[type='submit']:visible, .btn__primary--large:visible"
            )
            ...
            logger.info(
                "Credentials submitted successfully. Waiting for authentication / 2FA..."
            )
            return True
...
async def check_login_success(
    ...
    while True:
        try:
            ...
            has_nav_element = (
                await page.locator(
                    ".global-nav__me, header.global-nav, button:has-text('Me')"
                ).count()
                > 0
            )
...
async def main(state_path: str | Path = DEFAULT_SAVE_PATH):
    ...
    logger.info(
        f"Connecting to CloakBrowser container via CDP endpoint at {cdp_url}..."
    )
        ...
        try:
            ...
            if "/feed" not in page.url:
                ...
                if email and password:
                    ...
                    logger.warning(
                        "LINKEDIN_EMAIL or LINKEDIN_PASSWORD missing in .env"
                    )

            logger.info("If 2FA or CAPTCHA appears, please complete it manually.")
            success = await check_login_success(
                page, context, state_path=save_path, timeout=3600.0
            )
            if success:
                logger.info(
                    f"LinkedIn authentication state successfully saved to '{save_path}'!"
                )
            else:
