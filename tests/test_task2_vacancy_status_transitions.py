...
def test_get_eligible_vacancies_status_filter(db_session):
    """Query test that returns only created and review_error for review queue."""
    v1 = create_vacancy_direct(db=db_session, title="Created Job", text="Text 1", credentials="hr1@company.com")
    v2 = create_vacancy_direct(db=db_session, title="Error Job", text="Text 2", credentials="hr2@company.com")
    v3 = create_vacancy_direct(db=db_session, title="Reviewed Job", text="Text 3", credentials="hr3@company.com")
    v4 = create_vacancy_direct(db=db_session, title="Rejected Job", text="Text 4", credentials="hr4@company.com")
...
def test_valid_vacancy_status_transitions(db_session):
    ...
    v = create_vacancy_direct(db=db_session, title="Backend Dev", text="Text", credentials="hr@company.com")
...
def test_transition_with_allowed_from_statuses_validation(db_session):
    ...
    v = create_vacancy_direct(db=db_session, title="DevOps", text="Text", credentials="hr@company.com")
    assert v.status == VacancyStatus.created
...
def test_terminal_status_protection(db_session):
    ...
    for term_status in terminal_statuses:
        v = create_vacancy_direct(db=db_session, title=f"Job {term_status.value}", text="Text", credentials="hr@company.com")
...
def test_update_vacancy_with_status_kwarg(db_session):
    ...
    v = create_vacancy_direct(db=db_session, title="PLC Dev", text="Text", credentials="hr@company.com")
    updated = update_vacancy(db_session, v.id, title="Updated Title", status=VacancyStatus.rejected)
