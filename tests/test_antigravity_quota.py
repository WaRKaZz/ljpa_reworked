...
def test_usage_http_returns_gemini_five_hour_remaining_fraction(monkeypatch):
    ...
    monkeypatch.setattr(harness_runner.urllib.request, "urlopen", lambda request, timeout: Response())

    assert harness_runner.get_gemini_quota_remaining("http://harness:8080/run-harness") == 0.08


def test_submission_stops_before_harness_when_gemini_five_hour_remaining_is_at_or_below_seven_percent(monkeypatch):
...
def test_submission_continues_when_gemini_five_hour_remaining_is_above_seven_percent(monkeypatch):
    from ljpa_reworked import main
    ...
    monkeypatch.setattr(main, "confirm_url_application_submitted", lambda **kwargs: None)
