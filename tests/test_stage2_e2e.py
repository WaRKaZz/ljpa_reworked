...
def test_stage2_jobspy_integration(db_session):
    ...
    with patch("ljpa_reworked.services.jobspy.scrape_jobs", return_value=mock_df):
        ...
        assert (
            record.text
            == "We are looking for a Senior Backend Engineer proficient in Python and SQLite."
        )
        assert record.submit_email == "jobs@techcorp.com"
        assert record.submit_url == "https://www.linkedin.com/jobs/view/12345/"
...
def test_stage2_direct_vacancy_integration(db_session):
    ...
    saved = save_vacancy(
        ...
        submit_url="https://www.linkedin.com/feed/update/urn:li:activity:777777/",
        submit_email="devops@example.com",
        db=db_session,
    )
    ...
    assert (
        v.submit_url == "https://www.linkedin.com/feed/update/urn:li:activity:777777/"
    )
