...
def test_resume_prompt_forbids_unsupported_credentials_and_claims():
    ...
    assert (
        "omit unsupported tools, responsibilities, metrics, dates, credentials, and outcomes"
        in text
    )
    assert (
        "Freely use the candidate profile, vacancy, and general domain information"
        not in text
    )
...
def test_email_prompt_uses_neutral_salutation_without_explicit_name():
    text = Path(
        "src/ljpa_reworked/crews/email_generation_crew/config/tasks.yaml"
    ).read_text()
